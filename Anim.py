...
class Frame:
    ...
    def __init__(self, file_format_validator, frame_bytes):
        self.metadata = FrameMetaData(file_format_validator, frame_bytes)
        self.tiles = frame_bytes[file_format_validator.OFFSET_START_TILES:] #The tiles that give shape to the frame.

class FrameMetaData: #To make it clear that it's not the frame itself.

    def __init__(self, file_format_validator, frame_bytes):
        self.x_length = frame_bytes[file_format_validator.OFFSET_WIDTH]
        self.y_length = frame_bytes[file_format_validator.OFFSET_HEIGHT]
...
class Anim: #Yes this could be AnimUtils. Or maybe FrameUtils, come to think of it. #Similar structure to TileUtils. You have the main class, which then uses data from other classes to do its stuff.
    ...
    def load_new_physics_id_value(self, new_physics_id):
        ...
        if len(self.createanims.physics_list[self.createanims.current_physics_id]) // 2 != len(anim.frame_ids):
            if not self.createanims.in_play_anim and not self.createanims.undo_redo.in_switch_branch: #Don't show it when restarting due to Stop Anim. #And not when in a switch branch either. Similar logic to Physics Manager window, don't show it on and off during a switch branch.
