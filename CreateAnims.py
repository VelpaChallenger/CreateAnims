...
class CreateAnims:
    ...
    def init_anim_window(self):
        ...
        frame_id_entry_menu = tkinter.Menu(self.frame_id_entry, tearoff=0)
        frame_id_entry_menu.add_command(label="New Frame ID", command=self.menupopup.frame_id_entry_append)
        self.frame_id_entry.bind("<Button-3>", lambda event: self.menupopup.create_pop_up(frame_id_entry_menu, event))
        ...
        physics_id_entry_menu = tkinter.Menu(self.physics_id_entry, tearoff=0)
        physics_id_entry_menu.add_command(label="New Physics", command=self.menupopup.physics_id_entry_append)
        self.physics_id_entry.bind("<Button-3>", lambda event: self.menupopup.create_pop_up(physics_id_entry_menu, event))
    ...
    def init_save_changes_window(self):
        ...
        self.save_changes_affected_files_label = tkinter.Label(self.save_changes_affected_files_frame, text="\n".join(sorted(list(set(("".join(self.undo_redo.affected_files).rstrip().split("\n")))))), justify="left", wraplength=440)
