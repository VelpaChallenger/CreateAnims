class EntryReturn:
    ...
    def physics_dialog_x_entry(self, event=None):
        ...
        new_x_physics_converted = self.createanims.anim.calculate_physics(new_x_physics)
        ...
        new_y_physics_converted = self.createanims.anim.calculate_physics(new_y_physics)
        ...
        old_x_physics_converted = self.createanims.anim.calculate_physics(old_x_physics)
        old_y_physics = physics[(2*self.createanims.physics_dialog_current_frame) + 1]
        old_y_physics_converted = self.createanims.anim.calculate_physics(old_y_physics)
        if (new_x_physics, new_y_physics) == (old_x_physics, old_y_physics): #Has to be exact match. If one of them is different, that's alright, it does count as UndoRedo.
            ...
            return
        self.createanims.undo_redo.undo_redo([self.createanims.anim.load_new_physics_value, self.createanims.physics_dialog_current_frame, old_x_physics, old_y_physics, old_x_physics_converted, old_y_physics_converted], [self.createanims.anim.load_new_physics_value, self.createanims.physics_dialog_current_frame, new_x_physics, new_y_physics, new_x_physics_converted, new_y_physics_converted])
